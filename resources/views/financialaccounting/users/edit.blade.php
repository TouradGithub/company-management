    ...
    <form class="user-form" action="{{route('users-company.store')}}" >
        ...
        <div class="form-group">
            ...
            <select id="user_type" name="user_type" required >
                ...
                <option {{$user->model_type == "COMPANY"?'selected':''}} value="company">تابع للشركة</option>
                <option {{$user->model_type == "BRANCH"?'selected':''}} value="branch">تابع لفرع</option>
            </select>
        </div>
        <div class="form-group" id="branch_select" style="display: none;">
            ...
            <select id="branch_id" name="branch_id" class="select2">
                ...
                @foreach ($branches as $branch)
                    <option {{($user->model_id == $branch->id && $user->model_type =="BRANCH")?'selected':''}} value="{{ $branch->id }}">{{ $branch->name }}</option>
                @endforeach
            </select>
        </div>
...
<script>
    ...
    const userPermissions = @json($userPermission);
    ...
    $(document).ready(function () {
        ...
        function showPermission() {
            ...
            $.each(permissionGroups, function (groupName, permissions) {
                ...
                permissions.forEach(function (permission) {
                    let isChecked = userPermissions.includes(permission) ? 'checked' : '';
                    groupHtml += `
                        <div class="permission-item">
                            <input type="checkbox" id="${permission}" name="permissions[]" value="${permission}" ${isChecked}>
        ...
        $('#saveUser').on('click', function () {
            ...
            if (!name || !email || (password && password !== confirmPassword)) {
                Swal.fire('خطأ', 'يرجى ملء جميع الحقول المطلوبة أو التأكد من تطابق كلمة المرور!', 'error');
                return;
            }
            ...
            $.ajax({
                url: "{{ route('users-company.update', $user->id) }}", // سيتم إرسال الـ ID للمسار الصحيح
                method: 'PUT',
                data: data,
                success: function (response) {
                    if (response.success) {
                        Swal.fire('نجاح', 'تم تحديث المستخدم بنجاح!', 'success');
                        window.location.href = `/users-company`;
                    } else {
                        Swal.fire('خطأ', response.message || 'حدث خطأ أثناء التحديث!', 'error');
                    }
                },
                error: function (xhr) {
                    let errorMessage = 'فشل الاتصال بالخادم!';
